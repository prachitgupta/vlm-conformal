...
Optimisation:
  Sequential convex programming (SCP) with obstacle-clearance linearisation:
    d_safe * ||d_now|| - d_now^T d(X) <= 0
  solved by projected gradient steps on a convexified penalty objective.
"""
...
K_obs       = 100.0        # obstacle repulsion gain for smooth distance barrier
EPS         = 0.5          # singularity guard [m²]
...
SCP_INNER_ITERS = 12       # projected-gradient iterations per convex sub-problem
SCP_EPS      = 1e-3        # stop when trajectory update is small
SCP_LR       = 0.05        # projected-gradient step size
SCP_LAMBDA   = 25.0        # penalty multiplier (constraints)
MIN_CLEARANCE = 0.8        # desired clearance [m] from nearest obstacle point
...
class MPCPlanner:
    """
    Real-time optimal MPC trajectory generator using SCP.

    It mirrors the MATLAB penalty formulation:
      objective + lambda * (constraint violations),
    with obstacle constraints convexified around the previous trajectory.
    """

    @staticmethod
    def _rollout(x0: np.ndarray, U: np.ndarray) -> np.ndarray:
        """Single-integrator rollout: x_{k+1}=x_k+Ts*u_k."""
        X = np.zeros((T_horizon + 1, 6), dtype=float)
        X[0] = x0
        for k in range(T_horizon):
            X[k + 1, :3] = X[k, :3] + Ts * U[k]
            X[k + 1, 3:] = U[k]
        return X

    @staticmethod
    def _project_speed_limits(U: np.ndarray) -> None:
        """Project each control vector onto ||u|| <= V_MAX."""
        for k in range(T_horizon):
            spd = norm(U[k])
            if spd > V_MAX:
                U[k] *= V_MAX / max(spd, 1e-9)

    @staticmethod
    def _tracking_position_gradient(X: np.ndarray, x_ref: np.ndarray) -> np.ndarray:
        """
        Gradient of sum_k 0.5*(x_k-x_ref)^T Q_pos (x_k-x_ref) wrt controls.
        Uses triangular dependency: u_j affects x_k for all k >= j+1.
        """
        grad = np.zeros((T_horizon, 3), dtype=float)
        pos_err = X[1:, :3] - x_ref[:3][None, :]
        for j in range(T_horizon):
            tail = np.sum(pos_err[j:], axis=0)
            grad[j] = Ts * (Q_diag[:3] * tail)
        return grad

    @staticmethod
    def _terminal_state_gradient(X: np.ndarray, x_ref: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        """Return gradients of terminal position/velocity squared-error wrt controls."""
        pos_term_err = X[-1, :3] - x_ref[:3]
        vel_term_err = X[-1, 3:] - x_ref[3:]

        grad_pos = np.zeros((T_horizon, 3), dtype=float)
        for j in range(T_horizon):
            grad_pos[j] = 2.0 * Ts * pos_term_err

        grad_vel = np.zeros((T_horizon, 3), dtype=float)
        grad_vel[-1] = 2.0 * vel_term_err
        return grad_pos, grad_vel

    @staticmethod
    def _smooth_obstacle_gradient(X: np.ndarray, x_min: np.ndarray) -> np.ndarray:
        """Gradient of smooth barrier sum_k K_obs/(||x_k-x_min||^2 + EPS)."""
        grad = np.zeros((T_horizon, 3), dtype=float)
        g_pos = np.zeros((T_horizon, 3), dtype=float)
        for k in range(T_horizon):
            diff = X[k + 1, :3] - x_min
            d2 = float(diff @ diff)
            g_pos[k] = -2.0 * K_obs * diff / ((d2 + EPS) ** 2)

        for j in range(T_horizon):
            grad[j] = Ts * np.sum(g_pos[j:], axis=0)
        return grad

    @staticmethod
    def _convexified_obstacle_penalty_gradient(
        X: np.ndarray,
        x_min: np.ndarray,
        X_now: np.ndarray,
    ) -> np.ndarray:
        """
        SCP obstacle constraint from MATLAB formulation, adapted to 3D:
          v_k = d_safe*||d_now_k|| - d_now_k^T d_k <= 0,  d_k = x_k - x_obs
        penalty adds sum max(0, v_k)^2 and returns gradient wrt controls.
        """
        grad = np.zeros((T_horizon, 3), dtype=float)
        d_now = X_now[1:, :3] - x_min[None, :]
        d_now_norm = np.linalg.norm(d_now, axis=1)

        g_pos = np.zeros((T_horizon, 3), dtype=float)
        for k in range(T_horizon):
            dk = X[k + 1, :3] - x_min
            vk = MIN_CLEARANCE * d_now_norm[k] - float(d_now[k] @ dk)
            if vk > 0.0:
                g_pos[k] = -2.0 * vk * d_now[k]
    ...
    def solve(self,
        ...
        """
        U = U_warm.copy()
        self._project_speed_limits(U)
        X_now = self._rollout(x0, U)
        ...
        for _ in range(SCP_MAX_OUTER_ITERS):
            ...
            for _ in range(SCP_INNER_ITERS):
                ...
                grad = self._tracking_position_gradient(X, x_ref)
                grad_pos_term, grad_vel_term = self._terminal_state_gradient(X, x_ref)
                grad += SCP_LAMBDA * 0.1 * (grad_pos_term + grad_vel_term)

                # Soft penalty for control limit violations (subgradient).
                norms = np.linalg.norm(U, axis=1)
                limit_viol = norms - V_MAX
                for k in range(T_horizon):
                    if limit_viol[k] > 0.0:
                        grad[k] += SCP_LAMBDA * 2.0 * limit_viol[k] * (
                            U[k] / max(norms[k], 1e-9)
                        )

                if x_min is not None:
                    grad += self._smooth_obstacle_gradient(X, x_min)
                    grad += SCP_LAMBDA * self._convexified_obstacle_penalty_gradient(
                        X=X,
                        x_min=x_min,
                        X_now=X_now,
                    )
            ...
            X_new = self._rollout(x0, U)
            if np.max(np.abs(X_new - X_now)) < SCP_EPS:
                X_now = X_new
                break
            X_now = X_new

            # Stabilize SCP if inner solve diverges from previous iterate.
            U = 0.7 * U + 0.3 * U_prev
            self._project_speed_limits(U)
...
class MPCUAVNode(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info(
            f'MPC Optimal UAV Node started | Ts={Ts}s | horizon={T_horizon} | '
